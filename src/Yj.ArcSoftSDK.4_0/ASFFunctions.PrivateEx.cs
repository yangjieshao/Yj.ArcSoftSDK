...
namespace Yj.ArcSoftSDK._4_0
{
    public partial class ASFFunctions
    {
        ...
        private static ASF_MultiFaceInfo DetectFaceEx(IntPtr pEngine, IntPtr imageInfo)
        {
            ...
            {
                retCode = ASFFunctions_Pro_Win.ASFDetectFacesEx(pEngine, imageInfo, pMultiFaceInfo, (int)ASF_DetectModel.ASF_DETECT_MODEL_RGB);
            }
            else if (RuntimeInformation.IsOSPlatform(OSPlatform.Linux)
                  && RuntimeInformation.ProcessArchitecture == Architecture.X64)
            {
                retCode = ASFFunctions_Pro_Linux.ASFDetectFacesEx(pEngine, imageInfo, pMultiFaceInfo, (int)ASF_DetectModel.ASF_DETECT_MODEL_RGB);
            }
            else
            {
        ...
        private static byte[] GetSinglePersonFeatureEx(IntPtr pEngine, ASF_SingleFaceInfo faceInfo, IntPtr imageInfo, bool isRegister = true, bool hadMask = true)
        {
            ...
            if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows)
             && (RuntimeInformation.ProcessArchitecture == Architecture.X64
                || RuntimeInformation.ProcessArchitecture == Architecture.X86))
            {
                retCode = ASFFunctions_Pro_Win.ASFFaceFeatureExtractEx(pEngine, imageInfo, pSingleFaceInfo, (int)(isRegister ? ASF_RegisterOrNot.ASF_REGISTER : ASF_RegisterOrNot.ASF_RECOGNITION), hadMask ? 1 : 0, pFaceFeature);
            }
            else if (RuntimeInformation.IsOSPlatform(OSPlatform.Linux)
                  && RuntimeInformation.ProcessArchitecture == Architecture.X64)
            {
                retCode = ASFFunctions_Pro_Linux.ASFFaceFeatureExtractEx(pEngine, imageInfo, pSingleFaceInfo, (int)(isRegister ? ASF_RegisterOrNot.ASF_REGISTER : ASF_RegisterOrNot.ASF_RECOGNITION), hadMask ? 1 : 0, pFaceFeature);
            }
            else
            {
            ...
            try
            {
                ...
                if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows)
                 && (RuntimeInformation.ProcessArchitecture == Architecture.X64
                    || RuntimeInformation.ProcessArchitecture == Architecture.X86))
                {
                    retCode = ASFFunctions_Pro_Win.ASFImageQualityDetectEx(pEngine, imageInfo, pSingleFaceInfo, isMask ? 1 : 0, ref result, (int)ASF_DetectModel.ASF_DETECT_MODEL_RGB);
                }
                else if (RuntimeInformation.IsOSPlatform(OSPlatform.Linux)
                      && RuntimeInformation.ProcessArchitecture == Architecture.X64)
                {
                    retCode = ASFFunctions_Pro_Linux.ASFImageQualityDetectEx(pEngine, imageInfo, pSingleFaceInfo, isMask ? 1 : 0, ref result, (int)ASF_DetectModel.ASF_DETECT_MODEL_RGB);
                }
                else
                {
                    MemoryUtil.Free(ref pSingleFaceInfo);
