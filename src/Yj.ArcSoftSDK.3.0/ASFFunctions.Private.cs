    ...
    public partial class ASFFunctions
    {
        ...
        private static ASF_MultiFaceInfo DetectFace(IntPtr pEngine, ImageInfo imageInfo)
        {
                ...
                && RuntimeInformation.ProcessArchitecture == Architecture.X64)
            {
                if (IsPro)
                {
                    retCode = ASFFunctions_Pro_Linux.ASFDetectFaces(pEngine, imageInfo.Width, imageInfo.Height, (int)imageInfo.Format, imageInfo.ImgData, pMultiFaceInfo, 0x1);
                }
                else
                {
                    retCode = ASFFunctions_Linux.ASFDetectFaces(pEngine, imageInfo.Width, imageInfo.Height, (int)imageInfo.Format, imageInfo.ImgData, pMultiFaceInfo, 0x1);
                }
            }
            else
            {
        ...
        private static byte[] GetSinglePersonFeature(IntPtr pEngine, ASF_SingleFaceInfo faceInfo, ImageInfo imageInfo)
        {
                  ...
                  && RuntimeInformation.ProcessArchitecture == Architecture.X64)
            {
                if (IsPro)
                {
                    retCode = ASFFunctions_Pro_Linux.ASFFaceFeatureExtract(pEngine, imageInfo.Width, imageInfo.Height, (int)imageInfo.Format, imageInfo.ImgData, pSingleFaceInfo, pFaceFeature);
                }
                else
                {
                    retCode = ASFFunctions_Linux.ASFFaceFeatureExtract(pEngine, imageInfo.Width, imageInfo.Height, (int)imageInfo.Format, imageInfo.ImgData, pSingleFaceInfo, pFaceFeature);
                }
            }
            else
            {
        ...
        private static ASF_AgeInfo AgeEstimation(IntPtr pEngine)
        {
                  ...
                  && RuntimeInformation.ProcessArchitecture == Architecture.X64)
            {
                ...
                {
                    retCode = ASFFunctions_Linux.ASFGetAge(pEngine, pInfo);
                }
            }
            else
            {
        ...
        private static ASF_GenderInfo GenderEstimation(IntPtr pEngine)
        {
                ...
                && RuntimeInformation.ProcessArchitecture == Architecture.X64)
            {
                ...
                {
                    retCode = ASFFunctions_Linux.ASFGetGender(pEngine, pInfo);
                }
            }
            else
            {
        ...
        private static ASF_Face3DAngle Face3DAngleDetection(IntPtr pEngine)
        {
                  ...
                  && RuntimeInformation.ProcessArchitecture == Architecture.X64)
            {
                ...
                {
                    retCode = ASFFunctions_Linux.ASFGetFace3DAngle(pEngine, pInfo);
                }
            }
            else
            {
        ...
        private static ASF_LivenessInfo LivenessInfo_RGB(IntPtr pEngine)
        {
                  ...
                  && RuntimeInformation.ProcessArchitecture == Architecture.X64)
            {
                ...
                {
                    retCode = ASFFunctions_Linux.ASFGetLivenessScore(pEngine, pInfo);
                }
            }
            else
            {
        ...
        private static ASF_LivenessInfo LivenessInfo_IR(IntPtr pEngine, ImageInfo imageInfo, ASF_MultiFaceInfo multiFaceInfo)
        {
                  ...
                  && RuntimeInformation.ProcessArchitecture == Architecture.X64)
            {
                if (IsPro)
                {
                    retCode = ASFFunctions_Pro_Linux.ASFProcess_IR(pEngine, imageInfo.Width, imageInfo.Height, (int)imageInfo.Format, imageInfo.ImgData, pMultiFaceInfo, (int)FaceEngineMask.ASF_IR_LIVENESS);
                }
                else
                {
                    retCode = ASFFunctions_Linux.ASFProcess_IR(pEngine, imageInfo.Width, imageInfo.Height, (int)imageInfo.Format, imageInfo.ImgData, pMultiFaceInfo, (int)FaceEngineMask.ASF_IR_LIVENESS);
                }
            }
            else
            {
        ...
        private static IntPtr FaceInfoProcess(IntPtr pEngine, ImageInfo imageInfo, ASF_MultiFaceInfo multiFaceInfo, FaceEngineMask faceEngineMask)
        {
                  ...
                  && RuntimeInformation.ProcessArchitecture == Architecture.X64)
            {
                if (IsPro)
                {
                    retCode = ASFFunctions_Pro_Linux.ASFProcess(pEngine, imageInfo.Width, imageInfo.Height, (int)imageInfo.Format, imageInfo.ImgData, pMultiFaceInfo, (int)faceEngineMask);
                }
                else
                {
                    retCode = ASFFunctions_Linux.ASFProcess(pEngine, imageInfo.Width, imageInfo.Height, (int)imageInfo.Format, imageInfo.ImgData, pMultiFaceInfo, (int)faceEngineMask);
                }
            }
            else
            {
