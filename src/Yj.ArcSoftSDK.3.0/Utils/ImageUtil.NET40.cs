...
namespace Yj.ArcSoftSDK.Utils
{
    ...
    internal static partial class ImageUtil
    {
        ...
        public static ASVL_OFFSCREEN GetImageData(Image inImage)
        {
            ...
            var data = image.LockBits(new System.Drawing.Rectangle(0, 0, image.Width, image.Height), ImageLockMode.ReadOnly, PixelFormat.Format24bppRgb);
            ...
            var bgr_len = result.pi32Pitch[0] * data.Height;
        ...
        public static ImageInfo ReadBMP(Image image)
        {
            ...
            var data = bm.LockBits(new System.Drawing.Rectangle(0, 0, bm.Width, bm.Height), ImageLockMode.ReadOnly, PixelFormat.Format24bppRgb);
            try
            {
                //位图中第一个像素数据的地址。它也可以看成是位图中的第一个扫描行
                var ptr = data.Scan0;
                ...
                //获取去除对齐位后度图像数据
                var line = result.Width * 3;
                var pitch = Math.Abs(data.Stride);
                var bgr_len = line * result.Height;
                var destBitArray = new byte[bgr_len];
        ...
        public static ImageInfo ReadBMP_IR(Image image)
        {
            ...
            try
            {
                ...
                //获取去除对齐位后度图像数据
                var line = result.Width;
                var ir_len = line * result.Height;
                var destBitArray = new byte[ir_len];
        ...
        internal static Image CheckImage(Image image)
        {
            ...
            if (image != null)
            {
                ...
                if (result.Width % 4 != 0
                    || result.Height % 4 != 0)
                {
                    var newImage = ScaleImage(result, result.Width - (result.Width % 4), result.Height - (result.Height % 4));
                    result.Dispose();
                    result = newImage;
                }
            }
