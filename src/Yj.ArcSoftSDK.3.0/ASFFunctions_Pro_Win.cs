...
namespace Yj.ArcSoftSDK
{
    ...
    internal static partial class ASFFunctions_Pro_Win
    {
        ...
        /// SDK动态链接库路径
        /// </summary>
        private const string Dll_PATH = "ArcLib\\3.1\\libarcsoft_face_engine.dll";
        ...
        /// <param name="imageQualityInfo">人脸图像质量检测结果</param>
        /// <param name="detectModel">预留字段 暂时使用 <see cref="ASF_DetectMode.ASF_DETECT_MODEL_RGB"/></param>
        /// <returns>调用结果</returns>
        [DllImport(Dll_PATH, CallingConvention = CallingConvention.Cdecl)]
        public static extern int ASFImageQualityDetectEx(IntPtr pEngine, IntPtr imgData, IntPtr detectedFaces, IntPtr imageQualityInfo, int detectModel);
        ...
        /// <returns>调用结果</returns>
        [DllImport(Dll_PATH, CallingConvention = CallingConvention.Cdecl)]
        public static extern int ASFFaceFeatureExtractEx(IntPtr pEngine, IntPtr imgData, IntPtr faceInfo, IntPtr faceFeature);
